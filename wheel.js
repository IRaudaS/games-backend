...
const WHEEL_VALUES = [500, 800, 1000, 1500, 2000, 2500, 'BANCARROTA', 'PIERDE_TURNO', 500, 1000, 1500, 2000];
const WHEEL_PHRASES = {
  'BASKETBALL NBA': ['LEBRON JAMES LOS ANGELES LAKERS', 'STEPHEN CURRY GOLDEN STATE', 'NIKOLA JOKIC DENVER NUGGETS'],
  'VIAJANDO POR ESPAÑA': ['LA SAGRADA FAMILIA BARCELONA', 'MUSEO DEL PRADO EN MADRID', 'LA ALHAMBRA DE GRANADA'],
  'RELACIONADO CON MR BEAST': ['MR BEAST REGALA DINERO', 'FEASTABLES CHOCOLATE BAR', 'ULTIMO EN SALIR GANA'],
  'PELICULAS MARVEL': ['SPIDERMAN NO WAY HOME', 'AVENGERS ENDGAME THANOS', 'GUARDIANES DE LA GALAXIA'],
  'DC COMICS': ['THE BATMAN ROBERT PATTINSON', 'JOKER ARTHUR FLECK', 'LIGA DE LA JUSTICIA SNYDER']
};

function safeJsonParse(data, defaultValue = null) {
  if (!data) return defaultValue;
  if (typeof data === 'object') return data;
  try { return JSON.parse(data); } catch (e) { console.error('Error parseando JSON:', e); return defaultValue; }
}
...
function checkLetter(phrase, letter) {
  ...
  return { found: count > 0, count: count, letter: letter };
}
...
async function saveWheelGameState(pool, game) {
  await pool.query(
    `UPDATE wheel_games SET revealed_letters = $1, current_player = $2, player_money = $3, game_status = $4, consonants_used = $5, vowels_used = $6, last_activity = CURRENT_TIMESTAMP WHERE id = $7`,
    [JSON.stringify(game.revealedLetters), game.currentPlayer, JSON.stringify(game.playerMoney), game.gameStatus, JSON.stringify(game.consonantsUsed), JSON.stringify(game.vowelsUsed), game.id]
  );
}
...
module.exports = function(app, pool) {
    ...
    app.post('/api/wheel/games', async (req, res) => {
        try {
            ...
            await pool.query(`INSERT INTO wheel_games (id, phrase, category, revealed_letters, current_player, player_money, consonants_used, vowels_used) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [gameId, phrase, selectedCategory, JSON.stringify([]), FIXED_PLAYERS_WHEEL[0], JSON.stringify(initialMoney), JSON.stringify([]), JSON.stringify([])]);
    ...
    app.get('/api/wheel/my-games/:player', async (req, res) => {
        try {
            ...
            const result = await pool.query(`SELECT id, category, current_player, player_money, game_status, last_activity FROM wheel_games WHERE game_status = 'playing' AND (current_player = $1 OR player_money::text LIKE '%"' || $1 || '"%') ORDER BY last_activity DESC`, [player]);
            const myGames = result.rows.map(row => {
                const playerMoneyObj = safeJsonParse(row.player_money, {});
                return { gameId: row.id, category: row.category, currentPlayer: row.current_player, isMyTurn: row.current_player === player, myMoney: playerMoneyObj[player] || 0, lastActivity: row.last_activity, status: row.game_status };
            });
    ...
    app.post('/api/wheel/games/:gameId/play', async (req, res) => {
        try {
            ...
            const game = {
                id: row.id, phrase: row.phrase, category: row.category, revealedLetters: safeJsonParse(row.revealed_letters, []), currentPlayer: row.current_player, playerMoney: safeJsonParse(row.player_money, {}), gameStatus: row.game_status, roundNumber: row.round_number || 1, consonantsUsed: safeJsonParse(row.consonants_used, []), vowelsUsed: safeJsonParse(row.vowels_used, [])
            };
            if (game.currentPlayer !== player) return res.status(400).json({ error: 'No es tu turno' });
            let gameResult = { success: false, message: '' };
            switch (action) {
                // ... (toda la lógica del switch case para las jugadas de WoF)
            }
