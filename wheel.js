...
module.exports = function(app, pool) {
    ...
    app.get('/api/wheel/my-games/:player', async (req, res) => {
        console.log(`[WoF Debug] - RUTA /my-games/:player INVOCADA.`);
        try {
            const { player } = req.params;
            console.log(`[WoF Debug] - Jugador recibido: ${player}`);
            
            console.log('[WoF Debug] - Ejecutando consulta a la base de datos...');
            ...
            console.log(`[WoF Debug] - Consulta a la BD exitosa. Se encontraron ${result.rows.length} filas.`);
            ...
            console.log('[WoF Debug] - Enviando respuesta JSON al frontend.');
            res.json({ games: myGames });

        } catch (error) {
            console.error('[WoF Debug] - ERROR en la ruta /my-games/:player:', error);
